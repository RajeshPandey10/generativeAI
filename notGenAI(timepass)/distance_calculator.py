...
frame=cv2.VideoCapture(0)  # Try camera index 0 first
...
while True:
    ...
    # Light green color detection in HSV
    lower_light_green=np.array([40,40,40])
    upper_light_green=np.array([80,255,255])

    mask=cv2.inRange(src=hsv,lowerb=lower_light_green,upperb=upper_light_green)
